...
func (c *cmdRepackWindows) command() *cobra.Command {
	...
	cmd.Flags().StringVar(&c.flagDrivers, "drivers", c.defaultDrivers, "Path to virtio windows drivers ISO file"+"``")
	cmd.Flags().StringVar(&c.flagWindowsVersion, "windows-version", "", "Windows version to repack, must be one of ["+strings.Join(c.supportedVersions, ", ")+"]``")
	cmd.Flags().StringVar(&c.flagWindowsArchitecture, "windows-arch", "", "Windows architecture to repack, must be one of ["+strings.Join(c.supportedArchitectures, ", ")+"]``")
